...
describe('exportCsv', () => {
  ...
  it('CSVの1行目がヘッダー行になっている', () => {
    ...
    expect(lines[0]).toBe('商品名,SKU,JANコード,カテゴリ,区分,販売定価(税抜),原価,税率,ロットNo,賞味期限,在庫数');
  });
  ...
  it('ロットのない商品は空のロットNo・賞味期限・在庫数0で1行出力される', () => {
    ...
    // 行フォーマット: 商品名,SKU,JANコード,カテゴリ,区分,販売定価,原価,税率,ロットNo(空),賞味期限(空),在庫数(0)
  ...
  it('賞味期限のないロットは賞味期限列が空になる', () => {
    ...
    expect(cols[4]).toBe('資材'); // 区分列
    expect(cols[5]).toBe(''); // 資材は販売定価を持たない
    expect(cols[7]).toBe(''); // 資材は税率を持たない
    expect(cols[9]).toBe(''); // 賞味期限列
  });
