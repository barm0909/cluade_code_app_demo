...
- `InboundPlan.unitPrice` (仕入単価, optional at entry, 0 = not set) is copied onto the `StockTransaction` that `receiveInboundPlan` writes (`unitPrice`/`supplierId`, both optional and only ever set on that one `入荷` row) — this is the only place those two `StockTransaction` fields get populated. `costHistoryRows(ledger, suppliers)` reads them back out to build the read-only 原価履歴 tab, the same "derive from the ledger" pattern as disposal loss and lot trace. It does **not** write back to `Product.costPrice`. See `docs/cost-history-feature.md`.
